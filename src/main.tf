...
resource "kubernetes_secret" "gitlab_docker_registry_credentials" {
  ...
  data {
    ".dockercfg" = "${var.gitlab_registry_dockercfg}"
  }
...
resource "kubernetes_secret" "k8s_secrets" {
  ...
  count = "${var.vault_path == "" ? 0 : 1}"
  data  = "${data.vault_generic_secret.k8s.data}"
}
...
resource "kubernetes_role_binding" "ci_deploy" {
  ...
  subject {
    api_group = "rbac.authorization.k8s.io"
    kind      = "User"
    name      = "${element(concat(google_service_account.ci_deploy.*.email, list(var.deploy_user)), 0)}"
  }
}
